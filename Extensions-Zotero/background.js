...
Zotero.Connector_Browser = new function() {
	var _tabInfo = {}; //Object
	var _incompatibleVersionMessageShown; //Fee
	var _injectTranslationScripts = [ //array
		"node_modules.js",
		"zotero_config.js",
		"zotero.js",
		"promise.js",
	...
	this.onTranslators = function(translators, instanceID, contentType, tab, frameId) {
		...
		_updateExtensionUI(tab); //update icons
	}
	...
	function _updateExtensionUI(tab) {
		if (Zotero.Prefs.get('firstUse') && Zotero.isFirefox) return _showFirstUseUI(tab); //FireFox icon
		chrome.contextMenus.removeAll();

		if (_isDisabledForURL(tab.url)) { //check status online Zotero.
			_showZoteroStatus();
			return;
		} else {
			_enableForTab(tab.id);
		...
		if (translators && translators.length) {
			_showTranslatorIcon(tab, translators[0]); //Show Icons Collation exsample "https://www.youtube.com/channel/UC7X-NVVdpGEVFGO_deo9NnA"
			_showTranslatorContextMenuItem(translators);
		} else if (isPDF) {
			_showPDFIcon(tab); //Show Icons PDF.
		} else {
			_showWebpageIcon(tab); //Show Icons Webpage.
		}
		
		if (isPDF) {

			_showPDFContextMenuItem(); //working pdf for click.
		} else {
			_showWebpageContextMenuItem(); //working pdf for Webpage.
		}
	...
	function _showWebpageIcon(tab) {
		chrome.browserAction.setIcon({ //select Icon webpage .
			tabId: tab.id,
	...
	function _showPDFContextMenuItem() {
		chrome.contextMenus.create({
			...
			onclick: function (info, tab) {
                // console.log("1", tab);
				Zotero.Connector_Browser._saveAsWebpage(tab); //send to func _saveAsWebpage.
			},
			contexts: ['all']
		});
	}
	...
	this._saveAsWebpage = function(tab, withSnapshot) {
		if (tab.id != -1) {
            console.log("ture",tab.id);
			return Zotero.Messaging.sendMessage("saveAsWebpage", [tab.title, withSnapshot], tab); //call method for save.@me
		}
		// Handle right-click on PDF overlay, which exists in a weird non-tab state
			...
			chrome.tabs.query(
				...
				function (tabs) {
					Zotero.Messaging.sendMessage("saveAsWebpage", tabs[0].title, tabs[0]); //To file translate_item.js
				}
			);
		}
	}
